...
#include "utility.h"
#include <vector>
void allocateParams(workForGranger &workArray,int numComps,int particleBlockSize,paramContainer params,std::vector<int> lagList,std::vector<std::complex<float>> AR)
{
  /*
  workArray.lworkVal = particleBlockSize*params.numFreqs*2*(numComps-1)*(numComps-1);
  cudaMalloc((void**)&workArray.Qdev, sizeof(float2)*particleBlockSize*numComps*numComps);
  cudaMalloc((void**)&workArray.rotatedModels, sizeof(float2)*numComps*numComps*particleBlockSize*params.numFreqs);
  cudaMalloc((void**)&workArray.wArray, sizeof(float2)*numComps*numComps*particleBlockSize*params.numFreqs);
  ...
  //cudaMalloc(&workArray.ARdev,sizeof(float2)*AR.size());
  //cudaMemcpy(workArray.ARdev,AR.data(),sizeof(float2)*AR.size(),cudaMemcpyHostToDevice);
  */
  return;
}
